...
const installedExtensions = process.env.CADDYFILE_VSIX_EXTENSIONS_DIR;
const installedUserData = process.env.CADDYFILE_VSIX_USER_DATA_DIR;
const installedSmoke = installedExtensions !== undefined && installedUserData !== undefined;

if (process.platform === "linux" && process.env.CADDYFILE_TEST_XVFB !== "1") {
  const child = spawnSync(
    "xvfb-run",
    ["-a", process.execPath, resolve(import.meta.dirname, "run-integration-tests.mjs")],
    {
      cwd: root,
      env: { ...createIsolatedVSCodeEnvironment(), CADDYFILE_TEST_XVFB: "1" },
      stdio: "inherit",
    },
  );
...
const temporaryUserData = installedSmoke
  ? undefined
  : await mkdtemp(resolve(tmpdir(), "vscode-caddyfile-integration-"));
const temporaryWorkspaceRoot = await mkdtemp(resolve(tmpdir(), "vscode-caddyfile-workspace-"));
