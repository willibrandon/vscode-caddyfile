...
if (process.platform === "linux" && process.env.CADDYFILE_DOCS_XVFB !== "1") {
  const child = spawnSync(
    ...
    {
      cwd: root,
      env: { ...process.env, CADDYFILE_DOCS_XVFB: "1" },
      stdio: "inherit",
    },
  );
...
const vsix = resolve(root, `dist/caddyfile-${manifest.version}.vsix`);
const version = process.env.VSCODE_VERSION ?? "stable";
const outputDirectory = resolve(root, "docs-site/src/assets");
const temporaryRoot = await mkdtemp(join(tmpdir(), "vscode-caddyfile-docs-"));
...
try {
  ...
  await replaceLine(page, 11, "\trev");
  await waitForSuggestion(page, "reverse_proxy");
  await capture(page, "completion.png");

  await page.keyboard.press("Escape");
  await goTo(page, 7, 5);
  await waitForHover(page, "Proxy requests");
  await capture(page, "hover.png");

  await page.keyboard.press("Escape");
  await replaceLine(page, 11, "\treverze_proxy localhost:3000");
  await waitForDiagnostics(page);
  await goTo(page, 11, 5);
  await runEditorCommand(page, "Quick Fix...");
  await page
    .locator(".action-widget:visible")
    .filter({ hasText: "Use reverse_proxy" })
