...
const vsix = resolve(root, process.env.VSIX_PATH ?? `dist/caddyfile-${manifest.version}.vsix`);
const remoteSshVersion = "0.124.0";
const temporaryRoot = await mkdtemp(join(tmpdir(), "vscode-caddyfile-remote-"));
const artifactDirectory = resolve(root, "dist/remote-smoke");
const container = `vscode-caddyfile-remote-${process.pid}`;
...
const probeVsix = resolve(temporaryRoot, "caddyfile-remote-smoke-probe.vsix");
const resultPath = "/home/vscode/workspace/.remote-smoke-result.json";
const remoteVsix = `/home/vscode/${basename(vsix)}`;
const remoteProbeVsix = "/home/vscode/caddyfile-remote-smoke-probe.vsix";
...
try {
  ...
  const sshProbe = ["-F", sshConfig, "-o", "BatchMode=yes", "caddyfile-ci", "true"];
  ...
  const settings = `${JSON.stringify(
    {
      ...
      "remote.SSH.remotePlatform": { "caddyfile-ci": "linux" },
  ...
  requireInstalledExtension(installed, "willibrandon.caddyfile-remote-smoke-probe@0.0.0");
...
async function copyLanguageServerLog(containerName, destination) {
  const { stdout } = await run("docker", [
    ...
    "f",
    "-path",
    "*willibrandon.caddyfile/Caddyfile Language Server.log",
  ]);
  const logs = stdout
    .split(/\r?\n/u)
    .map((line) => line.trim())
    ...
    .sort();
  const log = logs.at(-1);
  if (log === undefined) throw new Error("The remote Caddyfile output log was not created.");
  await run("docker", [
    "cp",
    `${containerName}:${log}`,
