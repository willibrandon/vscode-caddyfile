...
export async function runInstalledDesktopSmoke(options) {
  ...
  const exitCode = await run(
    ...
    {
      ...commandEnvironment,
      CADDYFILE_EXPECTED_INSTALLED_EXTENSION_PATH_PREFIX: extensionsDirectory,
      CADDYFILE_EXPECTED_INSTALLED_EXTENSION_VERSION: expectedIdentity.slice(
        expectedIdentity.lastIndexOf("@") + 1,
      ),
      CADDYFILE_VSIX_EXTENSIONS_DIR: extensionsDirectory,
      CADDYFILE_VSIX_USER_DATA_DIR: userDataDirectory,
