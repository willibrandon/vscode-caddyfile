...
const registryExtensionUrl = new URL("/api/willibrandon/caddyfile", registryOrigin);
...
async function verifyOpenVsxRelease() {
  ...
  const checksum = resolve(root, `dist/caddyfile-${manifest.version}.sha256`);
  const sha256 = createHash("sha256")
    .update(await readFile(vsix))
    .digest("hex");
  const recordedChecksum = await readFile(checksum, "utf8");
  if (recordedChecksum !== `${sha256}  caddyfile-${manifest.version}.vsix\n`) {
