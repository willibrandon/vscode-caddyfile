...
exports.activate = async function activate() {
  ...
  try {
    ...
    const extension = vscode.extensions.getExtension("willibrandon.caddyfile");
    assert.ok(extension, "willibrandon.caddyfile is not installed in the remote extension host");
    const extensionVersion = extension.packageJSON.version;
    assert.equal(typeof extensionVersion, "string");
    assert.equal(
      extension.extensionPath,
      `/home/vscode/.vscode-server/extensions/willibrandon.caddyfile-${extensionVersion}`,
    );

    const caddyfileUri = vscode.Uri.joinPath(folder.uri, "Caddyfile");
    const referencedUri = vscode.Uri.joinPath(folder.uri, "parts.caddy");
    const document = await vscode.workspace.openTextDocument(caddyfileUri);
    ...
    const diagnostics = await waitForDiagnostics(
      caddyfileUri,
      (items) => items.some((diagnostic) => diagnosticCode(diagnostic) === "unknown-directive"),
    ...
    assert.ok(languageServerProcess, "the Caddyfile language server is not running remotely");
