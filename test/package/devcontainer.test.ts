...
describe("development container", () => {
  it("pins the complete non-root toolchain", async () => {
    ...
    for (const tool of ["chromium", "openssh-client", "socat", "xauth", "xvfb"]) {
  ...
  it("isolates generated and platform-specific paths per container", async () => {
    ...
    for (const mount of volumes) {
      expect(mount.source).toMatch(/^vscode-caddyfile-\$\{devcontainerId\}-/u);
    }
  ...
  it("runs the full editor and real-Caddy matrix", async () => {
    ...
    expect(config.containerEnv?.["CADDY_BIN"]).toBe("/usr/local/bin/caddy");
  ...
  it("keeps every container image under digest review", async () => {
    ...
    for (const dependency of ["node", "caddy", "docker", "mcr.microsoft.com/devcontainers/base"]) {
