...
| `npm run test:remote`           | Exact VSIX and Caddy command through Remote SSH               |
...
`bash .devcontainer/verify.sh` runs the full matrix in the pinned container, including the real
Caddy process boundary and the exact packaged VSIX.
